...
创建证书命令具体可参考[创建证书签名申请](/docs/enos/zh_CN/2.0.8/security/x509_ca/creating_csr.html)。
...
获取到证书以后，将其保存为`edge.pem`。需等待约30秒后，证书方能生效。

### 步骤3：获取根证书

根据步骤2中返回的根证书地址，即`certChainURL`的值，下载CA根证书，保存为`cacert.pem`。
