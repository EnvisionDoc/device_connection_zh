...
1. 在 **添加功能** 页面，**功能类型** 中选择 **测点**，并配置如下信息：
   ...
   - **测点类型**：测点类型用于区别测点数据的不同特征。在存储数据时，不同类型的测点数据可分类存储；在读取数据时，对不同类型的数据提供不同的处理能力。对各测点类型的描述如下：
     ...
     .. note:: 如果已有测点数据存储到TSDB，测点类型不能修改。测点类型的变更会导致已存储的历史数据无法查询。更多详细信息，参考 [配置TSDB存储策略](/docs/data-asset/zh_CN/latest/configuring_tsdb_storage.html)。
