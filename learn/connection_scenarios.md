...
### 网关代理(子设备)连接场景<nonsmartdeviceconnection>
...
### 场景1.1：接入设备已注册，设备出厂烧录自身的三元组<scenario1_1>

该场景要求设备出厂的时已烧录在云端注册设备后得到的设备三元组，这种场景对于烧录要求较高，安全性最高，但是可操作性较低。

.. image:: ../media/connection_scenario_1.1.png

### 场景1.2：接入设备已注册，批量设备烧录相同的Product信息<scenario1_2>

解决针对1.1场景可操作性较低的情况，增加了1.2场景。即：
...
### 场景1.3：接入设备未注册，通过可插拔采集信息动态注册<scenario1_3>
...
### 场景2.1：接入子设备已注册，设备三元组已保存在Edge<scenario2_1>

.. image:: ../media/connection_scenario_2.1.png

原理与2.2是一样的，只是2.2当中edge烧录了SA，具备调用EnOS API的权限，所以可以通过API创建子设备。而在2.1场景当中，你需要事先在云端注册子设备，获取子设备三元组信息，并提前将子设备三元组信息烧录到edge中。

在EnOS Edge配置中心进行设备连接配置的时候，需要将接入设备与预先烧录的子设备三元组绑定。

2.1场景的配置复杂度要大于2.2的场景，2.2灵活性更好，但是2.1的安全性更高。从便捷的角度，可以考虑使用2.2场景。

### 场景2.2：接入的子设备未注册，通过Edge动态注册<scenario2_2>
