...
- **非智能设备**：- 不支持烧录固件，也不具备Wifi、3G或4G联网的能力。该场景下，需要通过Edge网关来采集设备的数据，设备经由Edge网关连接至IoT平台。Edge需要支持固件烧录和联网。通过这种解决方案连接的设备又称作_sub device_。网关用作子设备的代理，以帮助它们完成认证、登录和数据传输等操作。
